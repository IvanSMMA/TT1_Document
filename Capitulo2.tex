...
\textbf{Dimensiones de un embedding como una analogía con mapas}
...
\textbf{Entrenamiento de los modelos de embedding (usualmente con 768 dimensiones)}
...
\textbf{Técnicas clásicas de generación de embeddings}
...
\textbf{Técnicas modernas (embeddings contextuales)}
...
La similitud del coseno se seleccionó como la métrica óptima debido a sus propiedades, que la han consolidado como un estándar en sistemas basados en embeddings. Entre sus principales ventajas destacan \cite{refcos1, refcos2, refcos3}:
\begin{itemize}
...
A continuación, se describen los elementos esenciales que conforman la arquitectura Transformer, los cuales permiten comprender por qué se ha convertido en el estándar en modelos de PLN \cite{reftrans1}:
\begin{enumerate}
...
\textbf{Arquitectura general: encoder y decoder}

El Transformer original está compuesto por dos bloques principales \cite{reftrans1}:
\begin{itemize}
...
\begin{center}
    ...
    \captionof{figure}[Arquitectura General Transformer]{Arquitectura General de un Transformer, obtenido de \cite{reftrans1}.}  % Pie de foto manual
\end{center}
...
\textbf{Procesamiento general de un Transformer}
\begin{enumerate}
...
En términos generales, la arquitectura Transformer integra los componentes que se describen a continuación \cite{reftrans1}:
\begin{itemize}
...
Extensión de BERT adaptada para generar embeddings semánticos de oraciones con alta calidad \cite{refebd10}. Se caracteriza por:
\begin{itemize}
...
El estilo REST ofrece múltiples beneficios que explican su amplia adopción en la industria del software \cite{refapi6}:
\begin{itemize}
...
El Modelo C4 es un enfoque creado por Simon Brown, cuyo propósito es aportar claridad y consistencia a la documentación de arquitectura de software \cite{refc4-1}\cite{refc4-3}. Este modelo surgió como respuesta a la disminución en la calidad de los diagramas técnicos derivada del uso extendido de metodologías ágiles, donde frecuentemente se emplean diagramas improvisados, ambiguos o con notación poco clara \cite{refc4-1}. El Modelo C4 propone una estructura jerárquica que permite representar un sistema de software en distintos niveles de abstracción, facilitando la comunicación entre desarrolladores, arquitectos y otros interesados dentro de un proyecto \cite{refc4-2}\cite{refc4-2}.\\

El modelo busca proporcionar un lenguaje común para describir software, reduciendo malentendidos y mejorando la colaboración entre equipos técnicos y no técnicos \cite{refc4-2}\cite{refc4-3}. Para ello se basa en un conjunto de abstracciones y tipos de diagramas que permiten visualizar desde el panorama general del sistema hasta los detalles más específicos de su implementación.\\
...
\begin{itemize}
    \item Persona: Representa a los usuarios finales que interactúan con el sistema, humanos o no.
    \item Sistema: La unidad de mayor nivel que ofrece valor a los usuarios.
    \item Contenedor: Aplicaciones o almacenes de datos que componen un sistema; son ejecutables o desplegables de forma independiente.
    \item Componente: Módulos o bloques internos que conforman cada contenedor.
\end{itemize}
...
\begin{center}
    \includegraphics[width=0.95\textwidth]{Images/Cap 2/C4/C4_Diagrama_Contenedor.png}
    \captionof{figure}[Diagrama de Contenedores]{Diagrama de Contenedores, obtenido de \cite{refc4-1}.} 
\end{center}

Su propósito es visualizar la arquitectura principal de implementación, de manera similar a un plano general de la estructura interna del sistema.\\
...
\begin{center}
    \includegraphics[width=0.95\textwidth]{Images/Cap 2/C4/C4_Diagrama_Componentes.png}
    \captionof{figure}[Diagrama de Componentes]{Diagrama de Componentes, obtenido de \cite{refc4-1}.} 
\end{center}

Sirve para el análisis detallado de la organización interna y las dependencias locales.\\
