...
\begin{center}
    \includegraphics[width=0.7\textwidth]{Images/Cap 2/Niveles_Arquitectura_PLN.png}
...
\begin{center}
	\makebox[\textwidth]{%
		\includegraphics[width=0.95\textwidth]{Images/Cap 2/NER.png}
	}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{2.5cm}|p{6cm}|}
        \hline
        \textbf{Término con prefijo} & \textbf{Raíz/\textit{Stem}} & \textbf{Término con el que causaría confusión} \\
        \hline
...
En la práctica, los \textit{embeddings} suelen ser representaciones densas de alta dimensionalidad, generalmente entre 128 y 1024 dimensiones, lo que permite capturar matices complejos del significado lingüístico con un grado notable de precisión \cite{refebd2}.\\
...
\textbf{Dimensiones de un \textit{embedding} como una analogía con mapas}
...
\newpage
\textbf{Cómo los vectores representan el significado en los textos}

Los \textit{embeddings} permiten que un sistema automático comprenda el significado de un texto al reflejar patrones de uso y coocurrencia entre las palabras. Aquellos términos que suelen aparecer en contextos similares tienden a adquirir vectores parecidos. De esta manera, palabras como rey y reina se ubicarán cerca entre sí porque suelen relacionarse con conceptos comunes, como la monarquía o el poder \cite{refebd3}.\\
...
Si bien los modelos reales manejan estructuras mucho más complejas, este tipo de ejemplos ilustra cómo los \textit{embeddings} capturan no solo definiciones individuales, sino también las relaciones semánticas que existen entre los conceptos.\\

\noindent \textbf{\textit{Embeddings}: representaciones de alta dimensionalidad}

Los \textit{embeddings} actuales, como los generados por modelos avanzados de Google, OpenAI y otros sistemas basados en redes neuronales, suelen tener cientos o miles de dimensiones. Estas dimensiones no están definidas manualmente; surgen de manera automática durante el entrenamiento y cada una representa un aspecto abstracto del significado aprendido por el modelo \cite{refebd1}.\\
...
Por ejemplo, el siguiente vector podría representar el \textit{embedding} de la frase “La materia no se crea ni se destruye, solo se transforma”:\\

\begin{center}
    \includegraphics[width=0.45\textwidth]{Images/Cap 2/Embeddings/3_RepresentaciónEmbedding.png}
    \captionof{figure}[Representación Vectorial de un embedding]{Representación vectorial de un \textit{embedding}, obtenido de \cite{refebd1}.}  % Pie de foto manual
\end{center}
...
De este modo, cuando se genera el \textit{embedding} de otra frase con un significado parecido. Por ejemplo: “La energía no desaparece, únicamente cambia de forma”, ambos vectores tienden a aparecer muy próximos dentro del espacio semántico, ya que comparten patrones conceptuales y contextuales. Esta cercanía numérica refleja la similitud entre las ideas expresadas en ambas oraciones.\\

\textbf{Entrenamiento de los modelos de \textit{embedding} (usualmente con 768 dimensiones)}

Los modelos encargados de producir \textit{embeddings} se entrenan utilizando cantidades masivas de texto, que pueden incluir libros, artículos científicos, publicaciones web, código y diversos tipos de documentos. En los últimos años, este proceso ha escalado de forma significativa, con modelos que se entrenan sobre volúmenes de datos cada vez mayores \cite{refebd4}.\\
...
\begin{center}
    ...
    \captionof{figure}[Dimensiones de un embedding]{Dimensiones de un \textit{embedding}, obtenido de \cite{refebd1}.}  % Pie de foto manual
\end{center}

El número de dimensiones que tendrá cada \textit{embedding} se establece como un parámetro del modelo. Un valor muy común en modelos modernos es 768 dimensiones, aunque pueden existir variantes con más o menos dimensiones según los objetivos del sistema y la arquitectura utilizada. Este proceso de entrenamiento exige una enorme capacidad computacional y puede requerir varios días o incluso semanas para completarse, especialmente cuando se trabaja con grandes volúmenes de datos o modelos de última generación \cite{refebd4}.\\

Además de comprender el concepto general de los \textit{embeddings}, es importante reconocer que existen diversas técnicas para generarlos. Estas técnicas han evolucionado con el tiempo y pueden clasificarse en dos grandes categorías: los métodos clásicos, que producen representaciones estáticas, y los métodos modernos, que generan representaciones contextuales capaces de capturar matices más complejos del lenguaje. A continuación, se describen brevemente los modelos más representativos de cada categoría.\\

\textbf{Técnicas clásicas de generación de \textit{embeddings}}
...
Estos modelos producen \textit{embeddings} estáticos: cada palabra posee un único vector sin importar el contexto en el que aparece.\\

\textbf{Técnicas modernas (\textit{embeddings} contextuales)}

Con el desarrollo de arquitecturas más avanzadas, como redes bidireccionales y modelos transformers, surgieron métodos capaces de generar \textit{embeddings} que dependen del contexto de cada oración. Esto permite capturar significados más finos y relaciones más complejas entre las palabras. Entre los modelos destacados se encuentran:

\begin{itemize}
    \item \textbf{ELMo}, basado en redes LSTM bidireccionales que generan representaciones sensibles al contexto \cite{refebd8}.
    \item \textbf{BERT}, modelo basado en transformers y entrenado mediante \textit{Masked Language Modeling}, que produce representaciones contextuales profundas \cite{refebd9}.
    \item \textbf{Sentence-BERT}, una adaptación de BERT diseñada específicamente para generar \textit{embeddings} de oraciones altamente útiles en tareas de similitud semántica \cite{refebd10}.
\end{itemize}
...
La similitud coseno es una medida ampliamente utilizada para evaluar el grado de semejanza entre dos vectores dentro de un espacio multidimensional. A diferencia de otras métricas basadas en distancia, este método no considera la magnitud de los vectores, sino la dirección hacia la cual apuntan. Esta característica la vuelve especialmente útil en espacios de alta dimensionalidad, donde las distancias euclidiana o Manhattan pueden volverse menos representativas \cite{refcos1}.\\

El cálculo consiste en obtener el coseno del ángulo formado por dos vectores distintos de cero \cite{refcos1}. El resultado es un valor numérico comprendido entre –1 y 1, donde:
\begin{itemize}
...
En el ámbito del \textit{Machine Learning}, Procesamiento de Lenguaje Natural (PLN) e Inteligencia Artificial, la similitud del coseno resulta fundamental debido a que estos sistemas operan con datos representados como vectores (por ejemplo, \textit{embeddings de texto}). Al convertir palabras, frases o documentos en vectores numéricos, se pueden comparar semánticamente mediante esta métrica \cite{refcos2}.\\

Por ejemplo, un \textit{chatbot} puede transformar una consulta en un \textit{embedding}, compararlo con los vectores almacenados en una base de datos y emplear la similitud del coseno para identificar cuál respuesta es la más cercana en significado. Este proceso permite medir cuánta relación existe entre dos representaciones vectoriales y seleccionar la opción más adecuada \cite{refcos2}\cite{refcos3}.\\

\textbf{Definición matemática:}

La similitud del coseno se define a partir de la relación angular entre dos vectores. Matemáticamente, se expresa como \cite{refcos1}\cite{refcos2}:
...
La similitud coseno presenta características que la hacen especialmente útil en modelos basados en \textit{embeddings}:
...
En el proceso de selección de una métrica adecuada para comparar \textit{embeddings}, se evaluaron diversas alternativas empleadas comúnmente en tareas de análisis vectorial. Sin embargo, varias de ellas presentan limitaciones importantes cuando se trabajan datos de alta dimensionalidad o representaciones semánticas continuas, razón por la cual fueron descartadas:
\begin{itemize}
    \item \textbf{Distancia Euclidiana}: Su desempeño se ve afectado por la magnitud de los vectores, lo que genera resultados inconsistentes cuando los \textit{embeddings} tienen distintas escalas o longitudes.
...
La similitud del coseno se seleccionó como la métrica óptima debido a sus propiedades, que la han consolidado como un estándar en sistemas basados en \textit{embeddings}. Entre sus principales ventajas destacan \cite{refcos1, refcos2, refcos3}:
\begin{itemize}
...
\subsection{\textit{Transformers} y Modelos Pre-Entrenados}
Desde su presentación en 2017, la arquitectura \textit{Transformer} supuso un cambio definitivo en el desarrollo de modelos de aprendizaje profundo, especialmente en el ámbito del Procesamiento de Lenguaje Natural (PLN). Antes de su aparición, el PLN dependía en gran medida de arquitecturas secuenciales como las Redes Neuronales Recurrentes (RNN) y sus variantes (LSTM y GRU). Aunque efectivas para ciertas tareas, estas redes tenían dificultades para capturar dependencias de largo alcance y requerían procesar la información paso a paso, lo que limitaba su escalabilidad \cite{reftrans1}\cite{reftrans2}.\\

El trabajo \textit{“Attention Is All You Need”} propuso un enfoque completamente distinto: eliminar la recurrencia y reemplazarla por un mecanismo de atención capaz de modelar relaciones globales entre \textit{tokens} en una sola operación. Esta innovación permitió acelerar drásticamente el entrenamiento y mejorar la calidad contextual de los modelos, convirtiéndose en la base de sistemas modernos como BERT, GPT, T5 y otras variantes avanzadas \cite{reftrans2}.\\

A continuación, se describen los elementos esenciales que conforman la arquitectura \textit{Transformer}, los cuales permiten comprender por qué se ha convertido en el estándar en modelos de PLN \cite{reftrans1}:
\begin{enumerate}
    \item \textbf{\textit{Embeddings}}. Cada \textit{token} del texto se representa como un vector denso de alta dimensionalidad. Esto permite trabajar con significado numérico en lugar de cadenas de texto.
    ...
    \begin{center}
        \includegraphics[width=0.60\textwidth]{Images/Cap 2/Transformers/Transformers1.jpg}
        \captionof{figure}[Vectores Posicionales]{Vectores Posicionales, obtenido de \cite{reftrans1}.}  % Pie de foto manual
    \end{center}
    \item \textbf{Mecanismo de atención (\textit{Self-Attention})}: Este mecanismo permite que cada \textit{token} “preste atención” a otros \textit{tokens} de la secuencia para capturar relaciones contextuales. Se utilizan tres vectores: \textit{query}, \textit{key} y \textit{value}.
    \item \textbf{\textit{Multi-Head Attention}}: Permite al modelo enfocarse en diferentes partes del texto desde distintas perspectivas. Cada “cabeza” aprende un aspecto contextual distinto.
    \item \textbf{Capas \textit{Feed-Forward}}: Redes neuronales tradicionales que procesan los datos post-atención token por token.
...
\textbf{Arquitectura general: \textit{encoder} y \textit{decoder}}

El \textit{Transformer} original está compuesto por dos bloques principales \cite{reftrans1}:
\begin{itemize}
    \item \textbf{\textit{Encoder}}: procesa la entrada analizando relaciones entre los tokens.
    \item \textbf{\textit{Decoder}}: genera la salida, utilizando atención tanto a la entrada codificada como al contexto generado anteriormente.
\end{itemize}

\begin{center}
    \includegraphics[width=0.80\textwidth]{Images/Cap 2/Transformers/Transformers2.jpg}
    \captionof{figure}[Arquitectura General Transformer]{Arquitectura General de un \textit{Transformer}, obtenido de \cite{reftrans1}.}  % Pie de foto manual
\end{center}

Dependiendo del tipo de modelo, puede utilizarse solo una de estas partes \cite{reftrans1}:
\begin{itemize}
    \item \textbf{Modelos \textit{encoder-only}}: Por ejemplo, BERT, empleados para clasificación, extracción de características o \textit{embeddings}.
    \item \textbf{Modelos \textit{decoder-only}}: Como GPT, orientados a generación de texto.
    \item \textbf{Modelos \textit{encoder–decoder}}: Como T5, útiles para tareas de traducción o reformulación.
\end{itemize}

\textbf{Procesamiento general de un \textit{Transformer}}
\begin{enumerate}
    \item \textit{Tokenización} del texto de entrada.
    \item Conversión de \textit{tokens} a \textit{embeddings}.
    \item Suma de las codificaciones posicionales.
    \item Procesamiento mediante múltiples capas de \textit{self-attention} y \textit{feed-forward}.
    \item (En modelos de generación) aplicación de atención enmascarada para controlar el orden autoregresivo.
    \item Obtención del siguiente \textit{token} mediante una capa final \textit{softmax}.
\end{enumerate}

\vspace{0.8em}

En términos generales, la arquitectura \textit{Transformer} integra los componentes que se describen a continuación \cite{reftrans1}:
\begin{itemize}
    \item \textbf{\textit{Encoder}}: procesa y contextualiza el texto de entrada.
    \item \textbf{\textit{Decoder}}: produce la salida \textit{token} por token (no presente en BERT).
    \item \textbf{\textit{Self-Attention}}: cada palabra puede relacionarse con todas las demás en la secuencia.
    \item \textbf{\textit{Multi-Head Attention}}: permite analizar el texto desde diversas perspectivas simultáneamente.
    \item \textbf{\textit{Feed-Forward Networks}}: capas densas que refinan la representación.
    \item \textbf{\textit{Positional Encoding}}: incorpora información relativa al orden.
\end{itemize}

\vspace{0.8em}

Algunos modelos pre-entrenados basados en Transformers son:\\

\textbf{BERT (\textit{Bidirectional Encoder Representations from Transformers})}

\begin{itemize}
    \item Entrenado con grandes corpus como Wikipedia y BookCorpus (aprox. 3.3B palabras) \cite{refebd9}.
    \item Utiliza dos tareas principales durante el pre-entrenamiento:
    \begin{itemize}
        \item \textbf{\textit{Masked Language Model}}: predecir palabras ocultas dentro de una oración.
        \item \textbf{\textit{Next Sentence Prediction}}: aprender relaciones entre oraciones consecutivas.
    \end{itemize}
    \item Su naturaleza bidireccional lo hace ideal para tareas de comprensión del lenguaje.
\end{itemize}

\textbf{SENTENCE-TRANSFORMERS (BERT)}

Extensión de BERT adaptada para generar \textit{embeddings} semánticos de oraciones con alta calidad \cite{refebd10}. Se caracteriza por:
\begin{itemize}
    \item Uso de redes siamesas o \textit{triplet loss} para aprender similitud textual.
    \item Optimización específica para tareas de búsqueda semántica, \textit{clustering} y clasificación basada en similitud.
...
React Native es un \textit{framework} de aplicaciones móviles desarrollado por Meta (Facebook) que permite crear aplicaciones para iOS y Android empleando JavaScript y React \cite{ref59}.\\
...
\begin{itemize}
    ...
    \item \textbf{Actualizaciones rápidas}: gracias a \textit{EAS Update}, los cambios o parches pueden implementarse de manera inmediata, garantizando un rendimiento estable y eficiente en todas las plataformas.
\end{itemize}
...
\newpage
\section{API}
Las API (del inglés \textit{Application Programming Interfaces}) son mecanismos que permiten la comunicación entre dos componentes de \textit{software} mediante definiciones, protocolos y reglas de intercambio. Actúan como un contrato de servicio que especifica cómo deben comunicarse las aplicaciones a través de solicitudes y respuestas estructuradas \cite{refapi1}. En este modelo, el cliente envía la solicitud y el servidor la procesa y devuelve una respuesta, permitiendo la integración entre distintos sistemas \cite{refapi1}.\\

\textbf{Tipos de API según su funcionamiento}
...
\textbf{Clasificación por ámbito y uso}
...
\subsection{API REST}
La Transferencia de Estado Representacional (REST, por sus siglas en inglés) constituye un estilo arquitectónico que define un conjunto de principios para el diseño de interfaces entre sistemas. Su propósito es establecer reglas que permitan crear APIs capaces de comunicarse de forma eficiente, escalable y fiable, incluso en entornos distribuidos como Internet. Una arquitectura basada en REST se caracteriza por ser flexible, fácil de mantener y por ofrecer portabilidad entre distintas plataformas, lo que la vuelve una opción común para el desarrollo de servicios web modernos \cite{refapi5}.\\

\newpage
Cuando una API se implementa siguiendo estos principios, se le denomina API REST o API RESTful. Del mismo modo, los servicios web construidos bajo esta arquitectura reciben el nombre de servicios web RESTful. En la práctica, los términos REST API y RESTful API suelen emplearse como equivalentes \cite{refapi5}.\\

A continuación, se describen los principales lineamientos que definen el estilo REST \cite{refapi5}:
\begin{enumerate}
...
Las API diseñadas bajo el estilo REST facilitan este tipo de integración, ya que emplean estándares abiertos y bien definidos que permiten una comunicación eficiente, confiable y sencilla de mantener \cite{refapi6}.\\

\textbf{Principios fundamentales de REST}

Los servicios RESTful se basan en una serie de principios arquitectónicos que garantizan interoperabilidad, escalabilidad y simplicidad \cite{refapi6}:
...
\begin{enumerate}
    ...
    \item \textbf{Sin estado (\textit{Stateless})}: Cada petición enviada al servidor debe contener toda la información necesaria para procesarla. El servidor no almacena el estado de las interacciones previas, lo que mejora la escalabilidad.
...
\textbf{Métodos HTTP utilizados en REST}

Los servicios RESTful emplean los métodos del protocolo HTTP para definir la operación que se desea realizar sobre un recurso \cite{refapi6}:
...
\textbf{Códigos de estado HTTP}

Las respuestas de una API RESTful utilizan códigos de estado estándar para indicar el resultado de la operación \cite{refapi6}:

\begin{itemize}
    \item \textbf{2xx – Éxito}: Ejemplos comunes incluyen \textit{200 OK} para peticiones correctas y 201 Created para la creación de un nuevo recurso.
    \item \textbf{4xx – Errores del cliente}: Incluyen situaciones como \textit{400 Bad Request} cuando la petición está mal formada o \textit{404 Not Found} si el recurso solicitado no existe.
    \item \textbf{5xx – Errores del servidor}: Indican fallos internos, como \textit{500 Internal Server Error}.
\end{itemize}

El estilo REST ofrece múltiples beneficios que explican su amplia adopción en la industria del \textit{software} \cite{refapi6}:
\begin{itemize}
...
\section{Modelo C4 de Arquitectura de Software}
El Modelo C4 es un enfoque creado por Simon Brown, cuyo propósito es aportar claridad y consistencia a la documentación de arquitectura de \textit{software} \cite{refc4-1}\cite{refc4-3}. Este modelo surgió como respuesta a la disminución en la calidad de los diagramas técnicos derivada del uso extendido de metodologías ágiles, donde frecuentemente se emplean diagramas improvisados, ambiguos o con notación poco clara \cite{refc4-1}. El Modelo C4 propone una estructura jerárquica que permite representar un sistema de \textit{software} en distintos niveles de abstracción, facilitando la comunicación entre desarrolladores, arquitectos y otros interesados dentro de un proyecto \cite{refc4-2}\cite{refc4-2}.\\

El modelo busca proporcionar un lenguaje común para describir \textit{software}, reduciendo malentendidos y mejorando la colaboración entre equipos técnicos y no técnicos \cite{refc4-2}\cite{refc4-3}. Para ello se basa en un conjunto de abstracciones y tipos de diagramas que permiten visualizar desde el panorama general del sistema hasta los detalles más específicos de su implementación.\\

\textbf{Abstracciones del Modelo C4}\\
El C4 modela la arquitectura de un sistema utilizando cuatro tipos de elementos fundamentales \cite{refc4-3}:
\begin{itemize}
    \item \textbf{Persona}: Representa a los usuarios finales que interactúan con el sistema, humanos o no.
    \item \textbf{Sistema}: La unidad de mayor nivel que ofrece valor a los usuarios.
    \item \textbf{Contenedor}: Aplicaciones o almacenes de datos que componen un sistema; son ejecutables o desplegables de forma independiente.
    \item \textbf{Componente}: Módulos o bloques internos que conforman cada contenedor.
\end{itemize}

Estas abstracciones permiten estructurar adecuadamente la representación del sistema y constituyen la base para los diagramas en sus diferentes niveles \cite{refc4-1}\cite{refc4-3}.\\

El modelo está compuesto por una jerarquía de cuatro diagramas, cada uno dirigido a un público específico y con un objetivo claro \cite{refc4-1,refc4-2,refc4-3}.\\

\textbf{Nivel 1: Diagrama de Contexto del Sistema}\\
Describe el sistema principal y cómo este interactúa con los usuarios y otros sistemas externos. Proporciona una vista de alto nivel que ayuda a entender el alcance general y la relación del sistema con su entorno \cite{refc4-1, refc4-2,refc4-3}.\\

\begin{center}
    \includegraphics[width=0.95\textwidth]{Images/Cap 2/C4/C4_Diagrama_Contexto.png}
    \captionof{figure}[Diagrama de Contexto]{Diagrama de Contexto, obtenido de \cite{refc4-1}.} 
\end{center}

Este nivel es especialmente útil para audiencias técnicas y no técnicas, ya que muestra el panorama completo sin entrar en detalles internos.\\

\textbf{Nivel 2: Diagrama de Contenedores}\\
Amplía el diagrama de contexto mostrando las aplicaciones, bases de datos y servicios que componen el sistema. Incluye las principales decisiones tecnológicas y aclara cómo interactúan los contenedores entre sí \cite{refc4-1,refc4-2,refc4-3}.\\

\begin{center}
    \includegraphics[width=0.71\textwidth]{Images/Cap 2/C4/C4_Diagrama_Contenedor.png}
    \captionof{figure}[Diagrama de Contenedores]{Diagrama de Contenedores, obtenido de \cite{refc4-1}.} 
\end{center}

\textbf{Nivel 3: Diagrama de Componentes}\\
En este nivel se detalla la estructura interna de un contenedor, desglosándolo en componentes y mostrando cómo colaboran para cumplir funcionalidades específicas \cite{refc4-1,refc4-2, refc4-3}.\\

\begin{center}
    \includegraphics[width=0.71\textwidth]{Images/Cap 2/C4/C4_Diagrama_Componentes.png}
    \captionof{figure}[Diagrama de Componentes]{Diagrama de Componentes, obtenido de \cite{refc4-1}.} 
\end{center}

\textbf{Nivel 4: Diagrama de Código}\\
Representa la implementación interna de un componente, típicamente en forma de diagramas UML generados automáticamente. Este nivel es opcional y rara vez se utiliza, ya que detalla clases e interfaces específicas del código \cite{refc4-1, refc4-2, refc4-3}.\\

\begin{center}
    \includegraphics[width=0.95\textwidth]{Images/Cap 2/C4/C4_Diagrama_Codigo.png}
    \captionof{figure}[Diagrama de Código]{Diagrama de Código, obtenido de \cite{refc4-1}.} 
\end{center}

El modelo C4 promueve la creación de diagramas claros, evitando ambigüedades y asegurando que cada elemento incluya nombre, tipo, tecnología y una breve descripción \cite{refc4-1}. Además, recomienda acompañar los diagramas con una leyenda que explique colores, bordes o formas utilizadas para que el lector pueda interpretarlos de manera consistente.\\

Este enfoque se ha convertido en una herramienta valiosa para mejorar la comunicación, documentación, colaboración y toma de decisiones, ofreciendo una forma estructurada y consistente de representar arquitecturas complejas \cite{refc4-2}. Gracias a su simplicidad y adaptabilidad, permite visualizar y comprender rápidamente los sistemas, incluso en equipos de gran tamaño o con alta rotación técnica \cite{refc4-3}.

\section{Servicios en la Nube}
Los servicios en la nube son soluciones tecnológicas que permiten a usuarios y organizaciones acceder bajo demanda a recursos informáticos, aplicaciones y datos a través de internet, sin necesidad de poseer infraestructura o \textit{hardware} especializado. Estos servicios son administrados por un proveedor de nube, lo que permite a los usuarios obtener acceso flexible y económico a las herramientas que necesitan, sin asumir los costos ni la complejidad de su gestión \cite{refnub1}.\\
...
\begin{center}
    \includegraphics[width=0.7\textwidth]{Images/Cap 2/Nube.png}
...
\begin{itemize}
    \item Reducción de \textit{hardware} físico y costos operativos.
...
\begin{itemize}
    ...
    \item \textbf{\textit{Software}}: herramientas de administración, máquinas virtuales y entornos de análisis que facilitan el uso de los recursos virtualizados.
\end{itemize}

Los servicios en la nube ofrecen una amplia gama de beneficios, que van desde bajos costos iniciales hasta una mayor libertad de acceso al \textit{software}. Su naturaleza adaptable y escalable permite que se ajusten a diferentes proyectos y empresas de cualquier tamaño o sector. La diversidad de opciones disponibles brinda la posibilidad de seleccionar el tipo de servicio que mejor se alinee con las necesidades de cada proyecto, lo que convierte a la nube en una solución versátil y aplicable en prácticamente cualquier entorno \cite{refnub5}.
...
\begin{itemize}
    \item \textbf{\textit{Cloud computing}}: incluye modelos de servicio como IaaS, PaaS y SaaS, permitiendo escalar aplicaciones según la demanda.
    ...
    \item \textbf{Aplicaciones empresariales y móviles}: AWS ofrece herramientas para el desarrollo de apps, respaldo de datos (\textit{backup}), recuperación ante desastres (\textit{disaster recovery}), autenticación doble, IoT e inteligencia de negocios.
\end{itemize}

Entre los servicios más utilizados y representativos de la plataforma se encuentran \cite{refaws3}:
\begin{itemize}
    \item \textbf{Servicios informáticos}: Amazon EC2 (\textit{Elastic Compute Cloud}) proporciona potencia de cómputo escalable, y AWS Lambda permite ejecutar código sin necesidad de administrar servidores.
...
\section{GoEnhance AI}
GoEnhance AI es una herramienta basada en Inteligencia Artificial orientada al procesamiento avanzado de contenido visual. La plataforma integra funciones de conversión entre formatos audiovisuales, mejora y ampliación de imágenes, así como generación de video a partir de diversos tipos de entradas. Su enfoque principal es automatizar tareas complejas de edición mediante modelos de IA que permiten transformar y optimizar material multimedia con distintos niveles de detalle \cite{refgoen0, refgoen1, refgoen2}.\\

Las principales funciones de GoEnhance son:
\begin{itemize}
    \item \textbf{Conversión de video a video}: GoEnhance AI incluye mecanismos para transformar videos existentes a diversos estilos visuales, incluyendo estilos animados, arte en píxeles y representaciones planas. La herramienta permite modificar la apariencia completa de un video manteniendo la estructura del contenido original \cite{refgoen1, refgoen2}.
    \item \textbf{Mejora y ampliación de imágenes}: la plataforma ofrece capacidades de aumento de resolución y mejora de calidad de imágenes. Sus modelos permiten escalar imágenes hasta cuatro veces su tamaño original, reducir ruido y recuperar detalles, lo que resulta útil en procesos de restauración de fotografías o incremento de nitidez en imágenes de baja calidad \cite{refgoen0, refgoen1, refgoen2, refgoen3}.
    \item \textbf{Imagen a video}: GoEnhance puede generar videos a partir de imágenes, integrando elementos visuales estáticos dentro de secuencias animadas. Esta funcionalidad permite producir contenido consistente con el estilo gráfico de la imagen proporcionada \cite{refgoen1}.
    \item \textbf{Intercambio de rostros y animación de personajes}: entre sus utilidades se encuentran herramientas de reemplazo facial tanto en imágenes como en videos, así como sistemas para animar personajes partiendo de imágenes estáticas. Estas funciones se basan en modelos generativos que estiman la pose, la estructura y los movimientos necesarios para la animación \cite{refgoen2}.
    \item \textbf{Transformación y generación de contenido}: GoEnhance también puede generar videos a partir de texto o de combinaciones entre texto e imagen, expandiendo su uso hacia tareas de creación automatizada de contenido visual \cite{refgoen3}.
\end{itemize}

Las capacidades de GoEnhance AI permiten su uso en actividades como \cite{refgoen0}:
\begin{itemize}
    \item Conversión estilística de videos a formatos animados o artísticos.
    \item Escalado y restauración de imágenes deterioradas.
    \item Producción de contenido audiovisual consistente a partir de imágenes de referencia.
    \item Sustitución y manipulación facial en contenidos digitales.
    \item Generación de videos basados en instrucciones textuales.
\end{itemize}

\begin{center}
    \includegraphics[width=0.85\textwidth]{Images/Cap 2/GoEnhance.png}
    \captionof{figure}[Logo de GoEnhance]{GoEnhance AI, obtenido de \cite{refgoen0}.} 
\end{center}

Estas aplicaciones permiten integrar GoEnhance AI en flujos de trabajo que requieren automatización, mejora visual o generación de contenido, haciendo posible procesar material multimedia con una mínima intervención manual \cite{refgoen0, refgoen1, refgoen2, refgoen3}.
