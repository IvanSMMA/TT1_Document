% Portada para protocolo de presentación de tesis
% Laboratorio de cómputo inteligente
% github.com/irvingvasquez/ProtocoloPlantillaLatex
% 2016


\documentclass[letter]{letter}

\usepackage{graphicx}
\usepackage[textwidth=17cm,textheight=20cm]{geometry}

\begin{document}
	
	\thispagestyle{empty}
	
	\begin{minipage}[c][0.1\textheight][c]{0.2\textwidth}
		\begin{center}
			\includegraphics[width=1.9cm, height=2.5cm]{TT1/Images/logo_ipn.jpg}
		\end{center}
	\end{minipage}
	\begin{minipage}[c][0.1\textheight][t]{0.6\textwidth}
		\begin{center}
			{\scshape {\Large Instituto Polit\'ecnico Nacional}}
			\vspace{.3cm}
			\hrule height2.5pt
			\vspace{.1cm}
			\hrule height1pt
			\vspace{.3cm}
			{\scshape  {\large Escuela Superior de C\'omputo} }
		\end{center}
	\end{minipage}
	\begin{minipage}[c][0.1\textheight][c]{0.2\textwidth}
			\begin{center}
				\includegraphics[height=2cm]{TT1/Images/escudoESCOM.png}
			\end{center}
	\end{minipage}
	
	\vspace{1cm}
	
	\begin{minipage}[c][0.8\textheight][c]{0.2\textwidth}
		\begin{center}
			\hskip2pt
			\vrule width2.5pt height17cm 
			\hskip1mm
			\vrule width1pt height17cm \\     	
			\end{center}
	\end{minipage}
	\begin{minipage}[c][0.6\textheight][c]{0.65\textwidth}
	\begin{center}
	
	{\Large \textbf{Trabajo Terminal I}}
	\vspace{1cm}
	
{\large {B - 042}}\\[40pt]            
	Presenta:\\[5pt]
	\textbf{
	\begin{itemize}
		\item Sánchez Mancilla Ivan Emiliano
		\item Juarez Solano Juan Martín 
		\item Rojas Alarcón Sergio Ulises
	\end{itemize}
	}
	
	\vspace{1cm}
    
	\vspace{1cm}
	
	{\small Directores de Trabajo Terminal:}
	
	{\large \textbf{
    \begin{itemize}
		\item M. en C. Elizabeth Moreno Galván
		\item M. en C. Jaime Arturo Lara Cázares
	\end{itemize}
    }
    }
	
	\vspace{2cm}
	
	{Ciudad de México,}{ }{25 de Noviembre de 2024}
	\end{center}
	\end{minipage}


\end{document}
