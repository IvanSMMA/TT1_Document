...
\begin{itemize}
    ...
    \item \textbf{Artículo base:} \textit{Sentence-BERT} (Reimers \& Gurevych, 2019)
\end{itemize}
...
{\large \noindent \textbf{Sistema de embeddings: Generación de embeddings}}
\begin{lstlisting}[language=Python]
...
{\large \noindent \textbf{Paso 1: Conversión a minúsculas}} \\[4pt]
...
\noindent \textbf{Objetivo}
Convertir texto a lista de caracteres individuales para reproducción de videos de señas letra por letra.
...
%====================================================
\section{Etapa 2: Modelo de embeddings y similitud semántica}

\subsection{Selección del modelo}

\textbf{Modelo:} \texttt{paraphrase-multilingual-MiniLM-L12-v2}

\textbf{Características:}
\begin{itemize}
    \item Modelo MiniLM, versión ligera de BERT.
    \item 12 capas Transformer.
    \item 384 dimensiones de embedding.
    \item $\sim$33M parámetros.
    \item Tamaño: $\sim$120MB.
\end{itemize}

\textbf{Entrenamiento:}
\begin{itemize}
    \item Más de 50 idiomas.
    \item Tarea principal: detección de paráfrasis.
    \item Millones de pares de oraciones.
    \item Arquitectura siamés + contrastive loss.
\end{itemize}

\textbf{Ventajas:}
\begin{enumerate}
    \item Multilingüe (incluye español).
    \item Rápido (40 ms en CPU).
    \item Ligero.
    \item Optimizado para similitud semántica.
\end{enumerate}

%---------------------------------------------------

\subsection{Arquitectura del modelo Transformer}

\begin{verbatim}
Input: Texto "Necesito ayuda urgente"
  ↓
Tokenización: [CLS, necesito, ayuda, urgente, SEP]
  ↓
Token Embeddings (384 dims)
  ↓
...
12 Capas de Transformer:
    ...
    Layer Normalization
  ↓
Output: Embedding de oración (384 dims)
\end{verbatim}

\textbf{Self-Attention:}\\
Permite que cada palabra atienda a todas las demás:
\begin{itemize}
    \item ``banco'' atiende a ``cerrado'' $\Rightarrow$ banco financiero.
\end{itemize}

\textbf{Multi-Head Attention:}\\
Ocho cabezas analizan distintos aspectos:
\begin{itemize}
    \item Relaciones sintácticas.
    \item Relaciones semánticas.
...
La estrategia usada es \textbf{Mean Pooling}:

```python
sentence\_embedding = mean(token\_embeddings)
...
frases = ["Hola", "Buenos días", "¿Cómo estás?"]
embeddings = model.encode(
    frases,
    convert_to_numpy=True,      # Retornar NumPy array
    normalize_embeddings=True,  # Normalizar a longitud 1
...
norm = sqrt(0.5^2 + 0.3^2 + 0.2^2) = 0.616

# Normalizado
embedding_norm = [0.5/0.616, 0.3/0.616, 0.2/0.616]
                = [0.812, 0.487, 0.325]
norm_nuevo = sqrt(0.812^2 + 0.487^2 + 0.325^2) = 1.0
\end{lstlisting}
...
\subsection{Ventaja de la normalización}
...
Cuando los vectores están normalizados a norma 1:
...
\subsection{Cálculo de similitud coseno}
...
\begin{itemize}
    \item $\theta = 0^\circ \Rightarrow \cos(\theta) = 1$ (vectores idénticos)
    \item $\theta = 90^\circ \Rightarrow \cos(\theta) = 0$ (ortogonales)
    \item $\theta = 180^\circ \Rightarrow \cos(\theta) = -1$ (opuestos)
\end{itemize}
...
# Similitud entre query y todas las frases
similarities = cosine_similarity([query_embedding], embeddings)[0]

# Resultado: array de similitudes
# similarities = [0.95, 0.82, 0.67, ...]
\end{lstlisting}

\subsection{Complejidad}

\begin{itemize}
    \item Cálculo directo: $O(d \times n)$
    \item Con centroides: $O(d \times k)$
\end{itemize}

donde:

\begin{itemize}
    \item $d = 384$ (dimensiones)
    \item $n = 43$ (frases)
    \item $k = 3$ (grupos)
\end{itemize}
...
\begin{itemize}
    \item 43 frases: $\sim 0.5$ ms
    \item 3 centroides: $\sim 0.05$ ms (10× más rápido)
\end{itemize}

% ======================================================

\section{Re-ranking en dos fases}

\subsection{Motivación}

La búsqueda lineal escala mal:

\begin{itemize}
    \item 1000 frases: $\sim 12$ ms
    \item 10000 frases: $\sim 120$ ms
\end{itemize}

\subsection{Solución: búsqueda jerárquica}

\textbf{Fase 1: Coarse-grained}
\begin{itemize}
    \item Comparación únicamente contra centroides.
    \item Complejidad: $O(k)$.
    \item Se seleccionan los mejores grupos candidatos.
\end{itemize}

\textbf{Fase 2: Fine-grained}
\begin{itemize}
    \item Se compara solo dentro de los grupos candidatos.
    \item Complejidad: $O(n/k)$.
\end{itemize}
...
Ejemplo con $n=43$, $k=3$:

\[
O(3) + O(43/3) = O(17)
\]

\textbf{Reducción:} 60\% menos comparaciones.

\subsection{Escalabilidad}

Con 1000 frases y 10 grupos:

\[
O(10) + O(1000 / 10) = O(110)
\]
...
\begin{itemize}
    \item Pérdida de precisión: 1--2\%.
    \item Mejora en velocidad: 2--9×.
\end{itemize}

\textbf{Decisión:} Se adopta para datasets mayores a 100 frases.

% ======================================================

\section{Etapa 3: API REST y arquitectura}

\subsection{Endpoint 1: POST /buscar}
...
\subsection{Endpoint 2: GET /grupos}
...
\subsection{Endpoint 3: GET /grupos/\{grupo\}}

\begin{lstlisting}
{
  "grupo": "A",
  "frases": ["Ayuda, por favor", "Llama a la policía", ...]
}
\end{lstlisting}

\subsection{Endpoint 4: POST /deletreo}
...
\begin{lstlisting}
{
  "texto_original": "Carlos",
  "deletreo": ["C", "A", "R", "L", "O", "S"],
  "total_caracteres": 6
}
\end{lstlisting}

\subsection{Endpoint 5: GET /health}
...
\noindent \textbf{Ventajas principales:}
\begin{itemize}
    \item Validación automática de tipos.
    \item Valores por defecto definidos explícitamente.
    \item Documentación integrada mediante OpenAPI.
    \item Serialización y deserialización automática.
\end{itemize}

\subsection{Ejemplo de validación}

\begin{lstlisting}
# Request inválido
{"texto": 123}  # Error: texto debe ser str
...
\noindent \textbf{Códigos HTTP utilizados:}
\begin{itemize}
    \item 200 OK: Petición exitosa.
    \item 400 Bad Request: Datos inválidos.
    \item 404 Not Found: Recurso inexistente.
    \item 500 Internal Server Error: Error general del servidor.
    \item 503 Service Unavailable: Servicio no disponible.
\end{itemize}

Ejemplos de errores comunes:
\begin{lstlisting}
# Error 400: Texto vacío
if not request.texto.strip():
    raise HTTPException(status_code=400, detail="Texto vacio")

# Error 404: Grupo no existe
if grupo not in grupos:
    raise HTTPException(status_code=404, detail=f"Grupo '{grupo}' no encontrado")

# Error 503: Matcher no inicializado
if matcher is None:
...
\begin{lstlisting}
# Startup
logger.info("Inicializando matcher mejorado...")
...
\noindent \textbf{Niveles de log:}
\begin{itemize}
    \item DEBUG: Información de diagnóstico detallada.
    \item INFO: Eventos normales del sistema.
    \item WARNING: Eventos inesperados que no detienen el servicio.
    \item ERROR: Errores recuperables.
    \item CRITICAL: Fallas graves que requieren detener el servicio.
\end{itemize}

\subsection{Documentación automática}

FastAPI genera documentación OpenAPI de forma automática:

\begin{itemize}
    \item Swagger UI: \texttt{http://localhost:8000/docs}
    \item ReDoc: \texttt{http://localhost:8000/redoc}
    \item Esquema JSON: \texttt{http://localhost:8000/openapi.json}
\end{itemize}

Ventajas:
\begin{itemize}
    \item Permite pruebas interactivas directamente desde el navegador.
    \item La documentación siempre coincide con el código.
    \item Facilita la generación de clientes automáticos.
\end{itemize}
