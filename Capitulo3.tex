...
En esta sección se presenta el análisis de viabilidad y factibilidad del prototipo de aplicación móvil de apoyo para la traducción de español a Lengua de Señas Mexicana (LSM), empleando técnicas de Procesamiento de Lenguaje Natural (PLN) y generación de animaciones en video asistida por IA. El objetivo es evaluar si existen las condiciones técnicas, humanas, tecnológicas y financieras necesarias para el desarrollo exitoso del prototipo. 

En primer lugar, se analiza la viabilidad, considerando los conocimientos del equipo, las tecnologías disponibles, las condiciones de ejecución y el potencial de escalabilidad del prototipo. Posteriormente, se estudia la factibilidad, enfocándose en los recursos financieros, humanos y tecnológicos, así como en el tiempo estimado para completar el proyecto.\\
...
El equipo de desarrollo posee conocimientos en herramientas de IA, particularmente en Procesamiento de Lenguaje Natural (PLN), así como habilidades básicas en edición y curaduría de recursos audiovisuales. Si bien en etapas tempranas del proyecto se exploraron alternativas basadas en modelado y animación 3D orientada a señas (ver \textbf{\nameref{anexo:blender_unity}}), en el alcance actual del prototipo se optó por un enfoque más acotado y realista, basado en el uso de animaciones en video generadas mediante servicios de IA en la nube.\\

En concreto, se empleó la plataforma GoEnhance AI (GoEnhance AI - All-in-One AI Video Generator), la cual permite crear videos a partir de texto, imágenes o clips existentes utilizando modelos avanzados de generación de video (por ejemplo, Runway, Kling o Veo~3). Este enfoque reduce la complejidad técnica asociada al \textit{rigging}, al control de esqueletos 3D y a la integración de motores gráficos en tiempo real, permitiendo que el equipo concentre sus esfuerzos en el diseño del flujo de traducción, el procesamiento de texto y la integración de los videos de señas en la aplicación móvil.\\

Aunque la experiencia directa del equipo en la Lengua de Señas Mexicana (LSM) es limitada, se considera viable la correcta representación de las frases del prototipo mediante el uso de referencias bibliográficas, materiales audiovisuales especializados y la validación manual del contenido generado. La disposición para el aprendizaje continuo y la consulta con fuentes especializadas respalda la viabilidad técnica del proyecto en función de las capacidades actuales del equipo.

\subsubsection{Tecnologías disponibles}
Actualmente existen diversas tecnologías y herramientas que facilitan la creación de sistemas de apoyo a la traducción de texto a señas, tales como conjuntos de datos de señas en video, plataformas de generación de video con IA, \textit{frameworks} para el desarrollo de aplicaciones móviles y librerías para el procesamiento de lenguaje natural. En el presente proyecto se privilegiaron soluciones que reducen la carga de infraestructura y la complejidad de integración, aprovechando servicios en la nube y herramientas de desarrollo multiplataforma.\\

Las tecnologías consideradas y efectivamente utilizadas en el prototipo incluyen:

\begin{itemize}
	\item Conjuntos de datos y videos de señas correspondientes a frases seleccionadas en LSM, generados y/o adaptados específicamente para el prototipo.
	\item La plataforma GoEnhance AI, empleada para generar videos de señas a partir de descripciones textuales, lo que permite obtener recursos audiovisuales consistentes sin requerir un \textit{pipeline} local de modelado 3D ni motores de animación en tiempo real.
	\item \textit{Frameworks} de desarrollo móvil como React (19.0.0) y Expo SDK (54.0.0), que facilitan la construcción de una aplicación multiplataforma y la integración de videos de señas en la interfaz de usuario.
	\item Contenedores identificando las unidades desplegables (aplicación móvil React/Expo, API backend FastAPI con despliegue en Render PaaS, almacenamiento JSON/NPZ y un bucket de AWS S3 para el servicio de animaciones en video)
	\item Herramientas de Procesamiento de Lenguaje Natural (PLN) para el análisis, normalización y segmentación de frases en español, que permiten mapear las entradas de texto de los usuarios a las frases disponibles en el repositorio de señas.
\end{itemize}

Este conjunto de tecnologías, basado en servicios de IA ya entrenados y herramientas de desarrollo consolidadas, incrementa la viabilidad del proyecto al reducir la necesidad de infraestructura especializada (como GPUs dedicadas o motores de animación complejos) y al permitir que el equipo se enfoque en la lógica de negocio del prototipo y en la experiencia de usuario.
...
\noindent \textbf{Recursos financieros.}
...
\begin{itemize}
	\item \textbf{Enfoque académico:} incluye estimaciones de costos simbólicos aproximados del mercado laboral en México correspondientes al año 2025 \cite{ref63}, considerando el uso de recursos propios, herramientas gratuitas y asesorías puntuales. Este enfoque representa fielmente la ejecución del proyecto dentro de un marco universitario.
	
	\item \textbf{Enfoque exploratorio de comercialización:} presenta una simulación de costos reales basada en tarifas aproximadas de mercado laboral en México correspondientes al año 2025 \cite{ref63}, incluyendo sueldos profesionales, licencias, infraestructura tecnológica y servicios externos. Este análisis permite anticipar los requerimientos financieros de una futura etapa de comercialización.
\end{itemize}
...
Este informe presenta el análisis financiero asociado a la creación del prototipo. El objetivo es identificar y estructurar los costos necesarios para garantizar la factibilidad del proyecto en su etapa de construcción inicial.
...
Esta primera etapa abarca todas las actividades necesarias para el desarrollo inicial del prototipo: definición de objetivos, especificaciones técnicas, requisitos, diseño de animaciones en video, entre otras. Aquí se implementan técnicas de PLN para la segmentación de frases y la generación o curaduría de animaciones en video que representen las señas en LSM de manera comprensible para los usuarios.

\paragraph{\textbf{Despliegue del prototipo.}} 
Incluye acciones para poner en funcionamiento la aplicación móvil desarrollada con React (19.0.0) y Expo SDK (54.0.0) en dispositivos con Android 14, así como su publicación en un entorno de pruebas, configuración técnica, documentación y ejecución de pruebas piloto con usuarios. Esta retroalimentación permite ajustar y mejorar la experiencia de uso.
...
Este enfoque estructurado de desarrollo permite garantizar una planeación clara y eficiente, considerando los elementos técnicos, operativos y financieros necesarios para la ejecución del proyecto. Como medida preventiva ante posibles riesgos o imprevistos, se contempla una reserva de contingencia equivalente al 15\% del costo total estimado.\\
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{|p{7cm}|r|r|r|}
			...
			Renta de espacio de \textit{co-working} (para 4 personas) & \$9,000.00 & \$36,000.00 & \$108,000.00 \\ \hline
			Suscripción a plataformas de desarrollo (GitHub Copilot, AWS, etc.) & \$800.00 & \$3,200.00 & \$9,600.00 \\ \hline
			...
			Licencia de \textit{software} y/o servicios para animación y edición de video & \$1,500.00 & \$6,000.00 & \$18,000.00 \\ \hline
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{|p{7cm}|c|r|r|}
			...
			Capacitación \textit{online} (cursos: PLN, LSM, Nube, etc.) & 3 cursos & \$800.00 & \$2,400.00 \\ \hline
			Compra de recursos para animaciones (opcional) & 1 paquete & \$2,500.00 & \$2,500.00 \\ \hline
...
el equipo de cómputo utilizado corresponde a dispositivos personales de los integrantes del proyecto, por lo cual no se ha considerado un costo adicional en esta categoría. La compra de herramientas o recursos gráficos y audiovisuales es considerada opcional y dependerá de la necesidad de complementar el material disponible de manera gratuita o de libre acceso, tal como sucedió en la versión prototipo de este trabajo terminal.
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{|p{7cm}|c|r|r|}
			\hline
			\textbf{Recurso} & \textbf{Unidades} & \textbf{Costo unitario (MXN \$)} & \textbf{Costo total (MXN \$)} \\ \hline
			Equipo de cómputo profesional (para desarrollo y animaciones) & 4 & \$25,000.00 & \$100,000.00 \\ \hline
			...
			Paquete profesional con licencia comercial de animaciones & 1 & \$10,000.00 & \$10,000.00 \\ \hline
			Cursos y certificaciones profesionales (PLN, UX, IA, LSM y Unity) & 4 & \$3,000.00 & \$12,000.00 \\ \hline
		...
		\end{tabular}
	\end{adjustbox}
	\caption[Costos estimados de compras no recurrentes en un escenario de comercialización]{Costos estimados de compras no recurrentes en un escenario de comercialización hipotético, elaboración propia.}
	\label{tab:compras_no_recurrentes_comercial}
\end{table}

\noindent Estos montos representan un escenario de referencia para una posible etapa comercial, y no corresponden a gastos realizados durante el desarrollo académico del prototipo.
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{|p{5.5cm}|c|r|r|r|r|}
			...
			Desarrollador \textit{Full Stack Junior} (estudiante) & 3 & \$72,000.00 (\$24,000.00 c/u) & \$288,000.00 & -- & -- \\ \hline
			Asesoría en animaciones (\textit{freelance}) & 1 parcial & \$300.00 (por sesión) & \$1,200.00 (cuatro sesiones) & -- & -- \\ \hline
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{|p{5.8cm}|c|r|r|r|r|}
			...
			\textbf{Tipo} & & \textbf{Mensual (MXN \$)} & \textbf{4 meses (MXN \$)} & \textbf{Mensual (MXN \$)} & \textbf{A un año (MXN \$)} \\ \hline
			Desarrollador \textit{backend} / \textit{frontend} & 2 & \$28,000.00 & \$224,000.00 & \$16,000.00 & \$192,000.00 \\ \hline
			Especialista en PLN & 1 & \$30,000.00 & \$120,000.00 & \$15,000.00 & \$180,000.00 \\ \hline
			Diseñador de animaciones / animador & 1 & \$25,000.00 & \$100,000.00 & \$12,000.00 & \$144,000.00 \\ \hline
...
\begin{itemize}
	...
	\item Realizar pruebas piloto para validar la comprensión y fluidez de las animaciones en Lengua de Señas Mexicana (LSM).
...
El equipo de desarrollo está conformado por tres estudiantes de la carrera de Ingeniería en Inteligencia Artificial, con experiencia académica en proyectos de Ingeniería de \textit{software}. Esta preparación garantiza que el equipo cuenta con las competencias necesarias para realizar el análisis, diseño, implementación, prueba y validación del prototipo propuesto.\\

Adicionalmente, se contempla la posibilidad de realizar capacitaciones específicas sobre LSM, manejo de conjuntos de datos de señas y técnicas de animación 3D, con el objetivo de reforzar el conocimiento técnico necesario para los módulos especializados del sistema.\\

La asignación de tres integrantes al proyecto se fundamenta mediante una estimación del esfuerzo utilizando el modelo \textbf{COCOMO II (\textit{Constructive Cost Model II})}, específicamente en su modalidad \textit{Early Design Model}, apropiada para proyectos en etapas iniciales \cite{refcocomo}.\\
...
\begin{itemize}
	...
	\item \textbf{Tamaño} = Tamaño estimado del \textit{software} en miles de líneas de código (KLOC). Se considera un tamaño de $2$ KLOC (2,000 líneas de código).
...
\noindent \textbf{Recursos tecnológicos}\\
El equipo de trabajo dispone del equipo de cómputo necesario para el desarrollo, prueba y validación de la aplicación móvil. Además, se cuenta con acceso a las plataformas de desarrollo y a las herramientas de \textit{software} requeridas, tales como ambientes de programación, bibliotecas especializadas de procesamiento de lenguaje natural (PLN), etc. Esta disponibilidad de recursos tecnológicos garantiza las condiciones adecuadas para la implementación efectiva del prototipo.\\
...
\subsection{Arquitectura del sistema (modelo C4 y capas)}
\subsubsection{Arquitectura del sistema (modelo C4 y capas)}

La arquitectura de SignAI se documenta mediante el modelo C4 (\textit{Context, Containers, Components, Code}) propuesto por \cite{brown2018c4}, y adoptado en contextos académicos como complemento de UML por \cite{vazquez2020c4}. En este trabajo se implementan tres de sus cuatro niveles jerárquicos: \textbf{Contexto}, que establece el sistema como una caja negra dentro de su ecosistema; \textbf{Contenedores}, que identifican las unidades desplegables (aplicación móvil React/Expo, API \textit{backend} FastAPI con despliegue en Render PaaS, almacenamiento JSON/NPZ y servicio externo AWS S3); y \textbf{Componentes}, que detallan la estructura interna del backend mediante sus módulos principales (API \textit{Layer}, \textit{Matcher} PLN, \textit{Embeddings Service}, \textit{Preprocess Module} y  \textit{Groups Manager}). El cuarto nivel (\textit{Code}) se sustituye por un diagrama de clases UML, enfoque consistente con la postura de \cite{brown2018c4} y con la experiencia reportada por \cite{vazquez2020c4}, quienes señalan que el modelo C4 complementa a UML en lugar de reemplazarlo.\\

Adicionalmente, se incluyó un diagrama de arquitectura por capas que documenta la separación lógica presentación–servicios–datos, siguiendo los principios de \textit{Separation of Concerns} descritos por \cite{fowler2002patterns}. La decisión de implementar una arquitectura monolítica modular, en lugar de una basada en microservicios, responde a criterios de proporcionalidad arquitectónica: el sistema aborda un único dominio de negocio (procesamiento de lenguaje natural para búsqueda semántica en LSM), maneja un volumen de datos acotado (32 frases con \textit{embeddings} precomputados de aproximadamente 256~KB) y carece de requisitos que justifiquen la complejidad operacional de una arquitectura distribuida (como escalado independiente por componente, despliegue políglota o múltiples equipos trabajando en paralelo), tal como advierte la literatura especializada sobre los costes organizativos y técnicos de los microservicios en sistemas pequeños o de un solo dominio \cite{dragoni2017micro,aldebagy2018comparative,newman2021microservices}.\\

El diseño monolítico modular, con bajo acoplamiento entre componentes internos, facilita una posible migración futura a microservicios cuando las métricas de tráfico, el tamaño del \textit{dataset} o la complejidad organizacional lo justifiquen. Esta progresión documental, desde vistas de alto nivel (Contexto, Contenedores, Capas) hasta detalles de implementación (Componentes y diagrama de clases UML), permite una comprensión arquitectónica tanto para \textit{stakeholders} técnicos como no técnicos, en concordancia con las mejores prácticas de documentación de arquitectura de \textit{software} establecidas por \cite{bass2012software} y con las directrices del estándar ISO/IEC/IEEE~42010:2011 \cite{iso42010}, que recomiendan describir la arquitectura mediante múltiples vistas consistentes alineadas con las preocupaciones de los distintos interesados.

\subsubsection{Diagrama de contexto (C4 nivel 1)}
\begin{center}
	\includegraphics[width=0.4\textwidth]{Images/Cap 3/contexto_presentacion.png}
	\captionof{figure}[Diagrama de contexto (C4 nivel 1)]{Diagrama de contexto (C4 nivel 1), elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/2s3ua3mz}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}

\subsubsection{Diagrama de contenedores (C4 nivel 2)}
\begin{center}
	\includegraphics[width=0.65\textwidth]{Images/Cap 3/contenedores_presentacion.png}
	\captionof{figure}[Diagrama de contenedores (C4 nivel 2)]{Diagrama de contenedores (C4 nivel 2), elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/yydk6ct5}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}

\newpage
\subsubsection{Diagrama de componentes (C4 nivel 3)}
\begin{center}
	\includegraphics[width=0.4\textwidth]{Images/Cap 3/componentes_vf.png}
	\captionof{figure}[Diagrama de componentes (C4 nivel 3]{Diagrama de componentes (C4 nivel 3), elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/4pzpmh3w}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}

\subsubsection{Diagrama de arquitectura en capas (3 Tier)}
\begin{center}
	\includegraphics[width=0.4\textwidth]{Images/Cap 3/capas_presentacion.png}
	\captionof{figure}[Diagrama de arquitectura en capas (3 Tier)]{Diagrama de arquitectura en capas (3 Tier), elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/3vxtfj3j}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}
...
\subsection{Modelos UML de análisis y diseño}

\subsubsection{Diagrama de procesos}
\begin{center}
	\makebox[\textwidth]{%
		\includegraphics[width=1.2\textwidth]{Images/Cap 3/Procesos.png}
	}
...
\begin{center}
	\makebox[\textwidth]{%
		\includegraphics[width=0.94\textwidth]{Images/Cap 3/actividades.png}
	}
	\captionof{figure}[Diagrama de actividades del sistema]{Diagrama de actividades del sistema, elaboración propia.}
\end{center}

\newpage
\subsubsection{Diagrama de clases}
\begin{center}
	\includegraphics[width=0.4\textwidth]{Images/Cap 3/clases_presentacion.png}
	\captionof{figure}[Diagrama de clases]{Diagrama de clases, elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/34x9yrzx}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}

\subsubsection{Diagrama de secuencia}
\begin{center}
	\includegraphics[width=0.4\textwidth]{Images/Cap 3/secuencia_vf.png}
	\captionof{figure}[Diagrama de secuencia]{Diagrama de secuencia, elaboración propia. 
		Véase la versión SVG para mayor detalle en \href{https://tinyurl.com/bde4v2ak}{\textbf{\underline{este enlace}}}.}  % Pie de foto manual
\end{center}
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\linewidth}
		...
		\begin{tabular}{|c|p{12cm}|}
			...
			\textbf{Descripción} & 
			El usuario ingresa texto en el campo indicado de la aplicación. El sistema procesa la frase y determina si existe una traducción directa en LSM. Si la hay, se reproduce la animación correspondiente con un avatar 3D. En caso contrario, el sistema recurre al deletreo dactilológico. \\ \hline
			
			\textbf{Atributos del texto} & 
			Máximo 50 caracteres. Validación para evitar caracteres especiales o símbolos no admitidos. \\ \hline
			
			\textbf{Entrada} & 
			Texto proporcionado por el usuario. \\ \hline
			
			\textbf{Procesamiento} & 
			El módulo de procesamiento de lenguaje natural (PLN) analiza el texto y busca coincidencias en el conjunto de datos. Si no hay coincidencia, se segmenta y transforma en deletreo letra por letra. \\ \hline
			
			\textbf{Salida} & 
			Se muestra la animación en 3D de la traducción en LSM o, en su defecto, del deletreo dactilológico correspondiente. Incluye un mensaje visual de inicio: “Vamos”. \\ \hline
			
			\textbf{Precondiciones} & 
			El sistema debe estar encendido, funcional y el usuario debe encontrarse en la pantalla principal con acceso al campo de texto. \\ \hline
			
			\textbf{Postcondiciones} & 
			El usuario visualiza la animación correspondiente y puede ingresar una nueva frase para traducir. \\ \hline
			
			\textbf{Excepciones} & 
			Si el texto excede el límite o contiene caracteres no válidos, el sistema mostrará un mensaje de error basado en RN05 e invitará a corregir la entrada. \\ \hline
		\end{tabular}
	\end{adjustbox}
	\caption[Caso de uso 1]{Caso de uso 1, elaboración propia.}
	\label{tab:caso_uso_1}
\end{table}
...
\begin{center}
	...
	\captionof{figure}[Diagrama de caso de uso 1]{Diagrama de caso de uso 1, elaboración propia.}
\end{center}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|X|X|X|}
		\hline
		\textbf{Emergencia (3 a 4 palabras)} & \textbf{Saludo (1 a 3 palabras)} & \textbf{Expresiones de Mínima Comunicación (1 a 3 palabras)} \\ \hline
		Ayuda, por favor. & Hola. & Gracias. \\ \hline
		Llama a la policía. & Buenos días. & Soy Sordo. \\ \hline
		Necesito un médico. & Buenas tardes. & Sí. \\ \hline
		Estoy herido. & Buenas noches. & No. \\ \hline
		¿Dónde está el hospital? & ¿Cómo estás? & Bien. \\ \hline
		Es una emergencia. & Bienvenido. & Mal. \\ \hline
		¿Necesitas ayuda? & Encantado de conocerte. & Perdón. \\ \hline
		¿Dónde está la salida? & ¿Cómo te llamas? & Entiendo. \\ \hline
		¡Fuego! ¡Fuego! & Me llamo [nombre]. & No Entiendo. \\ \hline
		Estoy sangrando. & Nos vemos. & No lo sé. \\ \hline
		Alto. & Me voy. & \\ \hline
	\end{tabularx}
...
\begin{center}
    ...
    \captionof{figure}[Mockup de la pantalla de carga de la aplicación]{Mockup de la pantalla de carga de la aplicación, elaboración propia.}
\end{center}
...
\begin{center}
    ...
    \captionof{figure}[Mockup de la pantalla de ingreso de texto]{Mockup de la pantalla de ingreso de texto, elaboración propia.}
\end{center}
...
En caso de que no se le pueda asignar una traducción a la frase ingresada, se mostrará la pantalla de deletreo dactilológico, al igual que la Pantalla anterior para que el usuario sepa en que momento inicia la traducción se le mostrará un mensaje con la palabra “Vamos”. En esta pantalla se muestra el avatar realizando el deletreo de la frase ingresada por el usuario. Al igual que en la pantalla anterior, se vuelve a mostrar el cuadro de entrada de texto para ingresar un nuevo mensaje.

\begin{center}
	\includegraphics[width=0.5\textwidth]{Images/Cap 3/Paleta.png}
	\captionof{figure}{Paleta de colores de la aplicación, elaboración propia.}
\end{center}

Se seleccionó la siguiente paleta de colores para la aplicación, ya que usa colores representativos de la bandera de LSM, además de que los colores son agradables a la vista y no generan cansancio visual.
